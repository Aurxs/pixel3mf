...
生成后用 Perfect Pixel 自动检测实际网格，并只接受每个轴都在 `60–90`（含边界）内的结果。若首张生成图任一轴高于 `90`，必须从零生成一幅新图，不能编辑、引用、附带或通过最近图片上下文复用那张失败图；仍可使用原始需求、官方角色资料、项目风格参考和用户最初提供的参考图。后处理不强制缩放到 24×24 或 75×75。
...
- 生图提示词默认要求 `24×24` 逻辑像素风格；Perfect Pixel 只接受每轴 `60–90` 的实际网格并保留检测结果，不强制改成 24×24 或 75×75
