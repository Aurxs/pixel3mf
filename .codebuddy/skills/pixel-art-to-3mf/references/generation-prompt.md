...
Priority 1 — pixel language: Use the simplicity and low information density of a 24 × 24 pixel-art design as the visual prior, then render the finished source on one consistent coarse logical grid and enlarge with nearest-neighbor only. Match Images 1–3 for large uniform cell size, stair-stepped contours, and symbolic detail. Keep all boundaries on one grid. Use about 8–12 flat colors.
...
Official-character research rule: Use the verified research brief below for canonical identity, appearance, costume, palette, props, and signature motifs. Treat any unverified adaptation as inference. Do not copy logos, readable marks, slogans, or text from official sources.
