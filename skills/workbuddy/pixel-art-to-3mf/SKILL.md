...
## Shared project and Windows runtime

Use the current Pixel3MF project with this skill under `skills/workbuddy/pixel-art-to-3mf/`; no separate WorkBuddy branch or workspace is required. Command examples use POSIX syntax. On Windows, use `.venv/Scripts/python.exe`, PowerShell quoting, and one-line commands instead of backslash continuations. Keep this anime workflow separate from `general-pixel-art-to-3mf`.
...
- Run `.venv/bin/python tools/workbuddy_pixel3mf.py doctor` before the first task in a new installation. Do not make a paid generation request during diagnosis. Missing TokenHub/COS credentials do not block WorkBuddy native generation or direct conversion. Use `configure-keychain` only on macOS when explicitly configuring that optional provider; on Windows use the provider environment variables if that provider was requested. Never place a secret in a shell argument or project file.
