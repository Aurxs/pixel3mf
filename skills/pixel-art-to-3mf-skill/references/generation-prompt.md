...
Before accepting the source, run Perfect Pixel auto-detection as a diagnostic. The requested `24 × 24` is prompt guidance; do not require the detector to return 24. Prefer approximately `44–60` detected cells per axis. If either axis exceeds `60`, regenerate with larger blocks and fewer shapes. Do not resize or force a downstream grid. Values below `44` may be accepted when the character remains readable and matches the coarse references.
