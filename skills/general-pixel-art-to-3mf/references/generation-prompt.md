...
- [reference-corgi-pixel-style.png](../assets/reference-corgi-pixel-style.png)

This is the user's approved corgi after Perfect Pixel and background cleanup: a 26×28 logical grid, enlarged 8× with nearest-neighbor, with binary alpha. The [logical original](../assets/reference-corgi-logical.png) is retained for provenance; attach only the enlarged style image by default. It comes from the accepted `20260917_corgi_complete` run, not a raw generator candidate or a failed checkerboard attempt.

Use it only for readable coarse cells, flat color within each cell, stepped contours and simplified detail. Do not copy the dog, pose, tan/white palette or anatomy into an unrelated subject. Do not force every new subject to its 26×28 grid. User subject photos still control identity and distinctive markings, and user style choices take precedence. No anime character examples or old gradient examples are attached by default.
