...
- For a user source or prepared derivative with real transparency, use its existing alpha directly. Skip semantic segmentation when the cutout already identifies the subject. Partial alpha is allowed as input here.
...
Use `"$SKILL_DIR/scripts/refine_pixel.py" --png-only`. This calls automatic grid detection with center sampling and no forced square/grid. The flag omits the old anime-source 60–85 density check; it does not resize to a chosen grid. Apply this refinement for both finished PNG and general-subject 3MF requests.

For a prepared cutout, add `--binarize-alpha`: sampled logical cells with alpha >=128 become opaque, the others transparent. This happens after Perfect Pixel, not by thresholding or flattening the full-resolution original. Retain the original and metadata, including how many sampled cells had partial alpha. Do not assume alpha thresholding preserves every intended part; inspect the result.
