...
CORE_SKILL_DIR = PROJECT_ROOT / "skills" / "codex" / "pixel-art-to-3mf-skill"
WORKBUDDY_SKILL_DIR = PROJECT_ROOT / "skills" / "workbuddy" / "pixel-art-to-3mf"
GENERATION_PROMPT_PATH = WORKBUDDY_SKILL_DIR / "references" / "generation-prompt.md"
...
def doctor(config_path: str | Path | None = None) -> dict[str, Any]:
    ...
    required_skill_files = (
        "SKILL.md", "references/generation-prompt.md",
        "references/source-acceptance.md", "references/pixel-refinement.md",
        "references/lumina-conversion.md",
        "assets/reference-24x24-block-style.png",
        "assets/reference-coarse-density-a.png", "assets/reference-coarse-density-b.png",
        "assets/reference-action-interaction.png",
    )
    missing_skill_files = [
        name for name in required_skill_files if not (WORKBUDDY_SKILL_DIR / name).is_file()
    ]
    record(
        "workbuddy_skill", not missing_skill_files,
        "available" if not missing_skill_files else f"missing: {', '.join(missing_skill_files)}",
    )
    core_ready = all(
