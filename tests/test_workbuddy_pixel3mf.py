...
class WorkBuddyStateTests(unittest.TestCase):
    ...
    def test_doctor_remains_callable_when_runtime_imports_are_missing(self) -> None:
        ...
        self.assertTrue(report["checks"]["workbuddy_skill"]["ok"])
        self.assertEqual(
            report["workbuddy_candidate_import_ready"], report["core_ready"]
        )

    def test_doctor_reports_missing_packaged_skill_without_a_builder(self) -> None:
        with (
            tempfile.TemporaryDirectory() as tmp,
            patch.object(workbuddy, "WORKBUDDY_SKILL_DIR", Path(tmp)),
            patch.object(workbuddy, "_tokenhub_api_key", return_value=None),
            patch.object(workbuddy, "_cos_credentials", return_value=(None, None)),
            patch.object(workbuddy, "_doctor_api_ready", return_value=False),
        ):
            report = workbuddy.doctor()
        self.assertFalse(report["checks"]["workbuddy_skill"]["ok"])
        self.assertIn("SKILL.md", report["checks"]["workbuddy_skill"]["detail"])
